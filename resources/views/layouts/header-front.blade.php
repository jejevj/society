...
<head>
	...
	<meta property="og:title" content="Society Event" />
	<meta name="csrf-token" content="{{ csrf_token() }}">
	<meta property="og:site_name" content="Society Event" />
	...
	<style>
		/* Reset semua wrapper Metronic agar halaman publik full width */
		html, body {
			width: 100%;
			max-width: 100%;
			overflow-x: hidden;
...
<body id="kt_app_body" data-kt-app-header-fixed-mobile="true" data-kt-app-toolbar-enabled="true" class="app-default">
	...
	{{-- NAVBAR PUBLIK (full width, no Metronic page wrapper) --}}
	<div id="kt_app_header" class="app-header" style="position:fixed;top:0;left:0;right:0;z-index:1000;width:100%;" data-kt-sticky="true"
			...
			<div class="d-flex align-items-center flex-grow-1 flex-lg-grow-0 me-lg-15">
				<a href="{{ env('APP_ROUTE') }}" class="d-flex align-items-center">
					...
					<div class="ms-3 d-flex flex-column">
						<span class="fw-bold text-white fs-4 d-none d-md-block">Society Event</span>
						<span class="fw-bold text-white fs-4 d-block d-md-none">Society Event</span>
						<span class="fw-bold text-white fs-6 d-none d-md-block">Science Bank</span>
						<span class="fw-bold text-white fs-6 d-block d-md-none">Science Bank</span>
					</div>
				</a>
			</div>
				...
				<div class="app-header-menu app-header-mobile-drawer align-items-stretch" data-kt-drawer="true"
					...
					<div class="menu menu-rounded menu-active-bg menu-state-primary menu-column menu-lg-row menu-title-gray-700 menu-icon-gray-500 menu-arrow-gray-500 menu-bullet-gray-500 my-5 my-lg-0 align-items-stretch fw-semibold px-2 px-lg-0 justify-content-end w-100"
						...
						<div class="menu-item me-0 me-lg-2">
							<a href="{{ route('about') }}" class="menu-link">
								<span class="menu-title fs-5">
									<span class="fw-bold {{ isset($menu_aktif) && $menu_aktif == 'about' ? 'text-maroon-active' : 'text-white' }} hover:text-maroon-active transition-colors">
										About
									</span>
								</span>
							</a>
						...
						<div class="menu-item me-0 me-lg-2">
							<a href="{{ route('about') }}" class="menu-link">
								<span class="menu-title fs-5">
									<span class="fw-bold {{ isset($menu_aktif) && $menu_aktif == 'event' ? 'text-maroon-active' : 'text-white' }} hover:text-maroon-active transition-colors">
										Event
									</span>
								</span>
							</a>
						...
						<div class="menu-item me-0 me-lg-2">
							<a href="{{ route('about') }}" class="menu-link">
								<span class="menu-title fs-5">
									<span class="fw-bold {{ isset($menu_aktif) && $menu_aktif == 'paper' ? 'text-maroon-active' : 'text-white' }} hover:text-maroon-active transition-colors">
										Paper
									</span>
								</span>
							</a>
								...
								<script>
									$(document).ready(function () {
										$("#btnLogout").on("click", function (e) {
											e.preventDefault();
											Swal.fire({
												title: "Konfirmasi", text: "Yakin ingin logout?", icon: "warning",
												showCancelButton: true, confirmButtonColor: "#d33", cancelButtonColor: "#3085d6",
												confirmButtonText: "Ya, Logout!"
											}).then((result) => {
												if (result.isConfirmed) {
													$.ajax({
														url: "{{ route('logout-backend-action') }}", type: "POST",
														data: { _token: "{{ csrf_token() }}" },
														success: function (res) {
															if (res.status) {
																Swal.fire("Berhasil!", res.message, "success").then(() => { window.location.href = "{{ route('login') }}"; });
															} else { Swal.fire("Gagal!", res.message, "error"); }
														},
														error: function () { Swal.fire("Error!", "Terjadi kesalahan, coba lagi.", "error"); }
													});
												}
											});
										});
	...
	{{-- Konten halaman dimulai di sini, tanpa wrapper Metronic --}}
