...
.ev-card-img { width: 100%; height: 130px; object-fit: cover; background: #e9ecef; }
...
.btn-retry {
    background: #E62020; color: #fff; border: none; border-radius: 8px;
    padding: 8px 16px; font-size: 0.82rem; font-weight: 700; cursor: pointer;
    display: flex; align-items: center; gap: 6px; white-space: nowrap; transition: background 0.2s;
}
    ...
    <div class="ev-card">
        ...
        <div class="ev-card-body">
            ...
            <div class="ev-card-meta">
                ...
                <span><i class="fa-solid fa-money-bill"></i> Rp {{ number_format($reg->total_bayar, 0, ',', '.') }}</span>
                @endif
            </div>
            ...
            <div class="retry-banner mt-2">
                ...
                <button class="btn-retry"
                    data-kode-registrasi="{{ $reg->kode_registrasi }}"
...
var _isProd    = {{ isset($midtransConfig) && $midtransConfig && ($midtransConfig->is_production ?? false) ? 'true' : 'false' }};
var _snapUrl   = _isProd
...
function loadSnapScript(callback) {
    ...
    s.onerror = function () { alert('Gagal memuat gateway pembayaran.'); };
    document.head.appendChild(s);
}

function doRetryPayment(btn) {
    var kodeEvent       = btn.dataset.kodeEvent;
    var kodeRegistrasi  = btn.dataset.kodeRegistrasi;
    ...
    // Gunakan route retryPayment yang sudah ada
    $.ajax({
        url: '{{ route("retryPayment") }}',
        type: 'POST',
        data: { _token: _csrf, kode_registrasi: kodeRegistrasi },
            ...
            loadSnapScript(function () {
                ...
                window.snap.pay(res.snap_token, {
                    ...
                    onPending: function () {
                        btn.disabled = false;
                        btn.innerHTML = '<i class="fa-solid fa-credit-card"></i> Bayar Sekarang';
                        setPayInline(kodeEvent, 'pending', '<div class="spinner-sm"></div><span>Pembayaran pending. Kembali ke sini setelah selesai.</span>');
                    },
                    onError: function (r) {
                        btn.disabled = false;
                        btn.innerHTML = '<i class="fa-solid fa-credit-card"></i> Bayar Sekarang';
                        setPayInline(kodeEvent, 'error', '<i class="fa-solid fa-circle-xmark"></i><span>' + (r.status_message || 'Pembayaran gagal.') + '</span>');
                    },
                    onClose: function () {
                        btn.disabled = false;
                        btn.innerHTML = '<i class="fa-solid fa-credit-card"></i> Bayar Sekarang';
                        setPayInline(kodeEvent, 'error', '<i class="fa-solid fa-circle-xmark"></i><span>Jendela ditutup. Klik Bayar Sekarang untuk mencoba lagi.</span>');
                    }
                });
            });
        },
